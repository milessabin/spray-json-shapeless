...
trait RichCompilerControl extends CompilerControl with RefactoringControl { self: RichPresentationCompiler =>
  ...
  def askSymbolInfoAt(p: Position): Option[SymbolInfo] = None

  def askTypeInfoAt(p: Position): Option[TypeInfo] = None


  def askTypeInfoById(id: Int): Option[TypeInfo] = None


  def askTypeInfoByName(name: String): Option[TypeInfo] = None


  def askTypeInfoByNameAt(name: String, p: Position): Option[TypeInfo] = None


  def askCallCompletionInfoById(id: Int): Option[CallCompletionInfo] = None

  def askPackageByPath(path: String): Option[PackageInfo] = None
  ...
  def askInspectTypeById(id: Int): Option[TypeInspectInfo] = None
//  askOr(typeById(id).map{ t => inspectType(t) }, t => None)

  def askInspectTypeAt(p: Position): Option[TypeInspectInfo] = None
  // askOr({//    reloadSources(List(p.source))
  //     inspectTypeAt(p)
  //   }, t => None)

  def askCompletePackageMember(path: String, prefix: String): Iterable[PackageMemberInfoLight] = List()
  // askOr({
  //     completePackageMember(path, prefix)
  //   }, t => List())

  def askCompleteSymbolAt(p: Position, prefix: String, constructor: Boolean): List[SymbolInfoLight] = List()
  // askOr({
  //     //    reloadSources(List(p.source))
  //     completeSymbolAt(p, prefix, constructor)
  //   }, t => List())

  def askCompleteMemberAt(p: Position, prefix: String): List[NamedTypeMemberInfoLight] = List()
  // askOr({
  //     //    reloadSources(List(p.source))
  ...
  def askUsesOfSymAtPoint(p: Position): List[RangePosition] = List()
  // askOr({
...
with RefactoringImpl with IndexerInterface {
  ...
  override val debugIDE: Boolean = false
  ...
  */
  ...
  //   askTypeAt(pos, new Response[Tree]())
  ...
  */
  ...
  def reloadAndTypeFiles(files: Iterable[SourceFile]) {}
