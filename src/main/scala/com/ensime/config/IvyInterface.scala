...
object ModuleID
{
  ...
  def apply(dep:DependencyDescriptor):ModuleID = {
    val mId = dep.getDependencyRevisionId
    ModuleID(mId.getOrganisation, mId.getName, mId.getRevision, dep.getModuleConfigurations.headOption, dep.isChanging, dep.isTransitive)
  }
...
{
  private[config] def mavenStyle(): Patterns = Patterns(ivyPatterns, artifactPatterns, true)
  private[config] def withIvys(patterns: Seq[String]): Patterns = Patterns(patterns ++ ivyPatterns, artifactPatterns, isMavenCompatible)
  private[config] def withArtifacts(patterns: Seq[String]): Patterns = Patterns(ivyPatterns, patterns ++ artifactPatterns, isMavenCompatible)
}
object Patterns
{
...
object Configurations
{
  ...
  private[config] val DefaultMavenConfiguration = defaultConfiguration(true)
  private[config] val DefaultIvyConfiguration = defaultConfiguration(false)
  private[config] def DefaultConfiguration(mavenStyle: Boolean) = if(mavenStyle) DefaultMavenConfiguration else DefaultIvyConfiguration
  private[config] def defaultConfiguration(mavenStyle: Boolean) = if(mavenStyle) Configurations.Compile else Configurations.Default
  private[config] def removeDuplicates(configs: Iterable[Configuration]) = Set(scala.collection.mutable.Map(configs.map(config => (config.name, config)).toSeq: _*).values.toList: _*)
}
