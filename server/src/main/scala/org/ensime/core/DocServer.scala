...
import akka.actor.{ Actor, ActorLogging, ActorRef, ActorSystem, Scheduler }
import akka.io.IO
...
class DocServer(
  ...
  var htmlToJar = HashMap[String, File]()

  val jarNameToJar = HashMap[String, File]()
  ...
  val docTypes = HashMap[String, DocType]()
  ...
  override def preStart(): Unit = {
    ...
    for (
      ...
      try {
        ...
        docTypes(jarFile.getName()) = Scaladoc
            ...
            if (entry.getName() == "index.html") {
              ...
              new String(bytes) match {
                case JavadocComment(version: String) if version.startsWith("1.8") =>
                  docTypes(jarFile.getName()) = Javadoc8
                case JavadocComment(_*) =>
                  docTypes(jarFile.getName()) = Javadoc
                case _ =>
              }
            }
          }
  ...
  private def fqnPackage(fqn: String): Option[String] = {
    PackRegexp.findFirstMatchIn(fqn).map { p =>
      p.group(1).stripSuffix(".")
    }
  }
